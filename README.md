...
```mermaid
flowchart TD
    START([User Query]) --> REWRITE[Query Rewrite Agent<br/>Resolves follow-ups & pronouns]
    REWRITE --> DETECT{Summary<br/>Request?}
    ...
    MAPREDUCE --> DONE([Save & Stream Result])
    
    DETECT -->|No| RESEARCH[Research Agent<br/>Vector similarity search in Qdrant<br/>Strictly scoped by user_id & document]
    RESEARCH --> SYNTH[Synthesis Agent<br/>Grounded answer with (page X) citations]
    SYNTH --> CRITIQUE[Critique Agent<br/>Strict JSON schema fact-checker]
    
    CRITIQUE --> DECISION{Passed?}
    DECISION -->|Yes| DONE
    DECISION -->|No, revisions < 3| RETRY[Widen top-k 8→10<br/>Trigger LangGraph retry loop]
    RETRY --> RESEARCH
    DECISION -->|No, revisions = 3| GIVEUP([Best-Effort Answer<br/>Tagged as Unverified])
    DECISION -->|API rate limit / 413 error| GIVEUP
...
| :--- | :--- | :--- |
| **Query Rewrite** | Groq `llama-3.1-8b-instant` | Rephrases conversational follow-up questions (*"what about that one?"*) into standalone search queries using prior turns. |
...
| **Synthesis Node** | `openai/gpt-oss-120b` (Groq) | Synthesizes comprehensive answers using strictly the retrieved context. Includes page citations `(page 3)`. |
...
| **Summarizer** | `openai/gpt-oss-120b` (Groq) | Executes map-reduce over ~4,500-word batches with exponential backoff for full-document synthesis. |
...
- **Problem**: In multi-user RAG, user A querying the vector store could accidentally retrieve chunks from user B, and two users uploading `report.pdf` would overwrite each other if point IDs were based on filename alone.
...
### 4. Memory Footprint Optimization (No PyTorch Dependency)
- **Problem**: Running heavy PyTorch embedding pipelines (`sentence-transformers`) routinely triggered out-of-memory crashes on resource-constrained containers (512MB RAM ceiling).
...
| :--- | :--- | :--- |
| **Faithfulness** | **0.954** | High grounding — virtually zero hallucinated claims. |
...
| **Context Recall** | **1.000** | Perfect extraction of all required source facts. |
...
cp .env.example .env # or configure:
...
Open [http://localhost:5173](http://localhost:5173) to enter the Axiom research workspace.

---

## License

Distributed under the MIT License. Developed for advanced multi-agent RAG research and document intelligence.
